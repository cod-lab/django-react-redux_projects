...
class RegisterAPI(generics.GenericAPIView):
    ...
    # when we make a request to register user on web, all the data(username,email,pass,headers,etc.) we send from web will be here
        ...
        # now the object saved in db will be stored into variable 'user' too
            # and saves user data received in db and,
            #which will now store in variable 'user'

        return Response({
                # return(send) user data and generated token to 'serializer'(leadmanager/accounts/serializers.py)
                # return(send) user data (not generated token) to 'serializer'(leadmanager/accounts/serializers.py)
                # return(send) user data (not generated token) to 'UserSerializer' in 'serializer'(leadmanager/accounts/serializers.py)
...
class LoginAPI(generics.GenericAPIView):
    ...
    # when we make a request to login user on web, all the data(username,pass) we send from web will be here
        ...
        # fn 'validate' in 'serializer' 'LoginSerializer' in file 'leadmanager/accounts/serializers.py' executes & returns here the above passed user data(unm,pass)
        # Now this data is stored in inbuilt 'property' 'validated_data' in from of dict
        
        # validated_data = an inbuilt property that holds dict, takes in returned data from a'serializer' 'LoginSerializer' stored in file 'leadmanager/accounts/api.py'
        nd        (bydefault) containg user data(unm,pass)     which is sent by 
        
        # 'LoginSerializer' returns the above passed data (which must be valid) and,
        # that data will be stored in variable 'user'
        ...
        # we can also send other stuff in 'Response'
            #"user": UserSerializer(user, context=self.get_serializer_context()).data,
            # calling 'UserSerializer' from 'serializer'(leadmanager/accounts/serializers.py) and,
            # passing object 'user' (having details: id,uname,email) to it and,
            # getting back same user details(id,uname,email) from 'UserSerializer' and converting it to serialized data(json format)
            # passing serialized(json format) user details to 'key' 'user'
            # (user, = user object
            # get_serializer_context() = Returns a dictionary containing any extra context that should be supplied to the serializer.
            # .data = 'UserSerializer' method which gives the serialized data(details in user object)
            #"token": AuthToken.objects.create(user)[1]
            # when a user login, a token is generated which is associated to that user only
            # that token is only valid for the period user has loged in
            # using 'AuthToken' which we imported above from 'knox'
            # AuthToken.objects.create(user) = passing object 'user' to 'AuthToken', now it'll create the token specific to that user, so when user makes any request from frontend, its gonna know who the user is from that token
            # that token goes in header in authorization part
        #})
