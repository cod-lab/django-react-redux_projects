...
# here, we didnt use 'ModelSerializer' bcz we are not creating a model, we are just validating that a user is authenticated or not by checking his credentials
    ...
    # (,'data') = getting user entered data at the time of login
        ...
        # It takes user credentials and checks them against each authentication backend, and returns a User object if the credentials are valid
        # It takes credentials as keyword arguments, username and password for the default case, checks them against each authentication backend, and returns a User object if the credentials are valid
        
        # converting user password to sha code using 'authenticate' which we imported above from 'django.contrib.auth'
