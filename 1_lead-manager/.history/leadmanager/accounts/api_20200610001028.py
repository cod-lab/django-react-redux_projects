...
class LoginAPI(generics.GenericAPIView):
    ...
    # when we make a request to login user on web, all the data(username,pass) we send from web will be here
        ...
        # fn 'validate' in 'serializer' 'LoginSerializer' in file 'leadmanager/accounts/serializers.py' executes & returns here the above passed user data in form of user object
        # Now this data is 
        # validated_data = an inbuilt property that holds the dict (bydefault) containg user data(unm,pass)     which is sent by 'serializer' 'LoginSerializer' stored in file 'leadmanager/accounts/api.py'
        ...
        # we can also send other stuff in 'Response'
            ...
            # that token goes in header in authorization part
        #})
        return Response({"user": LoginSerializer(user, context=self.get_serializer_context()).data})