...
export default function(state = initialState, action) {
    switch (action.type) {
        case USER_LOADING:
            return {                                            // send everything below to 'component'(leadmanager/frontend/src/components/ .js)
        ...
        case USER_LOADED:
            return {                                            // send everything below to 'component'(leadmanager/frontend/src/components/ .js)
                ...state,                                       // it includes all the data of variable 'initialState'
                isAuthenticated: true,                          // it means that we made a successful request to 'api' of the user wth the token
                isLoading: false,                               // it becomes false once the request dn or response is sent
                user: action.payload                                    // receive user as 'payload' from 'action'(leadmanager/frontend/src/actions/leads.js)
            };
        case AUTH_ERROR:
            localStorage.removeItem('token')                    // remove any token from local storage if there's one
            return {                                            
                // send everything below to 'component'(leadmanager/frontend/src/components/ .js)
