...
export const deleteLead = (id) => dispatch => {
    axios.delete(`/api/leads/${id}/`)                       // send 'delete' request to '/api/leads/' to delete that lead from the server(django)
        .then(res => {
            dispatch({
            // dispatch(send) 'deleteLead' to reducer('leadmanager/frontend/src/reducers/leads.js') to filter leads and display only remaining leads
                type: DELETE_LEAD,
                payload: id
            });
        }).catch(err => console.log(err));
...
export const addLead = (lead) => dispatch => {
    axios.post('/api/leads/', lead)                         // send 'post' request to '/api/leads/', adding a 'lead' into the server(django)
        .then(res => {
            dispatch({
            // dispatch(send) 'addLead' to reducer('leadmanager/frontend/src/reducers/leads.js') to add this lead to earlier leads and display them
                type: ADD_LEAD,
                payload: res.data
            });
        }).catch(err => console.log(err));
