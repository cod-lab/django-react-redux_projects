...
// sending      loaded user to server(django) & reducer
    ...
    // send 'get' request to '/api/auth/user' to get user from the server(django)
        .then(res => {
            dispatch({
            // dispatch(send) 'type' 'USER_LOADED' to 'reducer'(leadmanager/frontend/src/reducers/auth.js) to send user and,
            ...
            // dispatch(send) 'type' 'AUTH_ERROR' to 'reducer'(leadmanager/frontend/src/reducers/auth.js) to perform switch case 'AUTH_ERROR'
        });
}
