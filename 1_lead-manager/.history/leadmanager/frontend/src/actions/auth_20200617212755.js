    ...
    axios.get('api/auth/user', config)          // send 'get' request to 'api/auth/user' along with object 'config' (which holds the given token) to get user from the server(django)
        .then(res => {
            ...
            // 'reducer' sends it to 'component'(leadmanger/frontend/src/components/common/PrivateRoute.js) to display on web
                type: USER_LOADED,
                payload: res.data                                       // payload gets the user(id,username,email) that returned from server
            });
        ...
        // object 'config' here will hold the token returned from the server(django) to log-in
            .then(res => {
                ...
                // 'reducer' sends it to 'component'(leadmanger/frontend/src/components/common/Login.js) to login and redirect to 'Dashboard'
                    type: LOGIN_SUCCESS,
                    payload: res.data                                       // payload gets the token and user details(token,id,username,email) which returned from server
                });
