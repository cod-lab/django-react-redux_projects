...
import { createMessage } from './messages';                 // importing 'createMessage' from 'action'(leadmanager/frontend/src/actions/messages.js) to call it from this file
...
export const addLead = (lead) => dispatch => {              // sending stored 'lead' in 'prop' 'addLead' to server & reducer
    axios.post('/api/leads/', lead)                         // send 'post' request to '/api/leads/' to add a 'lead' into the server(django) and add it here only
        .then(res => {
            ...
            // call 'createMessage' from 'action'(leadmanager/frontend/src/actions/messages.js) and,
