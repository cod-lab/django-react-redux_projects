...
export const getLeads = () => dispatch => {
    axios.get('/api/leads/')                                // send 'get' request to '/api/leads/' to get all leads from the server(django)
        .then(res => {
            dispatch({
            // dispatch(send) 'getLead' to reducer('leadmanager/frontend/src/reducers/leads.js') to get all leadsand,
            // send to components('leadmanger/frontend/src/components/leads/Leads.js') to display all leads on web
                type: GET_LEADS,
                payload: res.data
            });
...
export const deleteLead = (id) => dispatch => {
    axios.delete(`/api/leads/${id}/`)                       // send 'delete' request to '/api/leads/' to delete that lead from the server(django)
        .then(res => {
            ...
            // send to components('leadmanger/frontend/src/components/leads/Leads.js') to display only remaining leads
                type: DELETE_LEAD,
                payload: id
            });
        }).catch(err => console.log(err));
...
export const addLead = (lead) => dispatch => {
    axios.post('/api/leads/', lead)                         // send 'post' request to '/api/leads/' to add a 'lead' into the server(django)
        .then(res => {
            ...
            // send to components('leadmanger/frontend/src/components/leads/Leads.js') to display them
                type: ADD_LEAD,
                payload: res.data
            });
        }).catch(err => console.log(err));
