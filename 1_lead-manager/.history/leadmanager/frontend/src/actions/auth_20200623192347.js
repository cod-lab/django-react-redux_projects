...
// dispatch(send) 'type' 'LOGOUT_SUCCESS' to call this 'switch case' from 'reducer'(leadmanager/frontend/src/reducers/auth.js)
// bringing in 'getState' which allows us to fetch something from the state
// here using this to get token if there's one
    ...
    const config = {                                                        // config object
     ...
     // Request Body
     //const body = ({username: null, password: null}); // convert JavaScript object into string and passing this along with 'post' request below
     //const body = ({});
    ...
    axios.post('api/auth/logout', null, config) // send 'get' request to 'api/auth/logout' along with object 'config' (which holds the given token) to get user from the server(django)
        .then(res => {
            dispatch({
                // dispatch(send) 'type' 'USER_LOADED' & 'payload' to 'reducer'(leadmanager/frontend/src/reducers/auth.js) to send user and,
                // 'reducer' sends it to 'component'(leadmanger/frontend/src/components/common/PrivateRoute.js) to display on web
