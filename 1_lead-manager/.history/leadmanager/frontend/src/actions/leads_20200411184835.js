...
// importing variable 'createMessage' from 'action'(leadmanager/frontend/src/actions/messages.js) to call it from this file
    ...
    axios.post('/api/leads/', lead)                         // send 'post' request to '/api/leads/' to add a 'lead' into the server(django) and add it here only
        ...
        .catch(err => {
            ...
            // 'reducer' sends it to component(leadmanger/frontend/src/components/leads/Alerts.js) to display apt. err on webpg for performed action
                type: GET_ERRORS,
                payload: getErrors                          // when any err is generated on srvr, it is sent to payload
            });
        });
};
