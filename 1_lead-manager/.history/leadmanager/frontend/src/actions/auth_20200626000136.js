...
// REGISTER USER
export const register = ({ username, email, password }) => dispatch => {              // sending object user to server(django) & reducer
    ...
    // Request Body
    const body = JSON.stringify({ username, email, password });         // convert JavaScript object into string and passing this along with 'post' request below

    axios.post('api/auth/register', body, config)
    // send 'post' request to 'api/auth/register' along with 'body'(holding user data) & 'config'(currently empty)
    // object 'config' here will hold the token returned from the server(django) to log in immediately
        .then(res => {
            dispatch({
            // dispatch(send) 'type' 'REGISTER_SUCCESS' & 'payload' to 'reducer'(leadmanager/frontend/src/reducers/auth.js) (to send user along with token) and,
            // 'reducer' sends it to 'component'(leadmanger/frontend/src/components/common/Register.js) to login automatically and redirect to 'Dashboard'
                type: REGISTER_SUCCESS,
                payload: res.data                                       // payload gets the data(token,id,username,email) which returned from server
            });
        }).catch(err => {                                               // if user unables to register (due to some problem) then
            dispatch(returnErrors(err.response.data, err.response.status));
            // call variable 'returnErrors' from 'action'(leadmanager/frontend/src/actions/messages.js) and,
            // pass the data(err.response.data, err.response.status) to it's parameters 'msg' & 'status' and,
            // dispatch(send) that variable to 'reducer'(leadmanager/frontend/src/reducers/errors.js)
            dispatch({ type: REGISTER_FAIL });
            // dispatch(send) 'type' 'REGISTER_FAIL' to 'reducer'(leadmanager/frontend/src/reducers/auth.js) to perform switch case 'REGISTER_FAIL' to get everything to default
        });
};
