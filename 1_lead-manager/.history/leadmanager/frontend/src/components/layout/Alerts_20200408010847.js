    ...
    componentDidUpdate(prevProps) {
        ...
        if (error !== prevProps.error) {
            if (error.msg.name)
                ...
                // (`name: ${error.msg.name.join()}`) = gives us the err(in array) that sever has generated, not the one we hv defined
                // ${} = this makes/add variable while printing
                // .join() = convert array to string
            if (error.msg.email)
                alert.error(`email: ${error.msg.email.join()}`);
                // (`email: ${error.msg.email.join()}`) = gives us the err(in array) that sever has generated, not the one we hv defined
                // ${} = this makes/add variable while printing
                // .join() = convert array to string                
        }
    }
