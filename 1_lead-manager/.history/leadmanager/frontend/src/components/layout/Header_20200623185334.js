...
export class Header extends Component {
    ...
    render() {
        ...
        const authLinks = (                                                         // variable to be displayed when logged in
            <ul className="navbar-nav ml-auto mt-2 mt-lg-0">
                <li className="nav-item"><button className="nav-link btn btn-info btn-sm text-light" onClick={this.props.logout}>Logout</button></li>
            </ul>
        );
        //const { username, password } = null;

        const guestLinks = (                                                         // variable to be displayed when logged out
            <ul className="navbar-nav ml-auto mt-2 mt-lg-0">
            ...
            <nav className="navbar navbar-expand-sm navbar-light bg-light">
                <div className="container">
                    ...
                    <div className="collapse navbar-collapse" id="navbarTogglerDemo01">
                        ...
                        { isAuthenticated ? authLinks : guestLinks }                {/* if 'isAuthenticated' is true then 'authlinks' otherwise 'guestLinks' */}
                </div>
            </nav>
        )
    }
...
// 'logout' = 'prop' = created above
    // cls 'Header' is a 'component' here wrapped thru 'connect' using '()'
    // cls 'Header' is a 'component' here wrapped with 'action' 'logout' thru 'connect' using '()'
