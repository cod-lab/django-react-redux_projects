    ...
    componentDidUpdate(prevProps) {
        ...
        if (error !== prevProps.error) {
            ...
            if (error.msg.message) // if message err prompted from srvr
                ...
                // ${} = this makes/add variable while printing
                // .join() = convert array to string
            if (error.msg.non_field_errors) // if message err prompted from react-redux
                alert.error(error.msg.non_field_errors.join());
                // (error.msg.message.join()) = gives us the err(in array) that sever has generated, not the one we hv defined for any prob
                // .join() = convert array to string
            if (error.msg.username) // if email err prompted from srvr
