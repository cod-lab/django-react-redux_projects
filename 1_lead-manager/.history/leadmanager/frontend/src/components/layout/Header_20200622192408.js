...
export class Header extends Component {
    ...
    fn4() {<Redirect to='/login' />};
    // static fn3 = (this.props.logout);
    //fn2() { window.location.reload(false); }
    //fn3() { (this.props.logout); }
    fn1() { const fn3 = (this.props.logout); return <Redirect to='/login' />; }

    render() {
        const { isAuthenticated, user } = this.props.auth;

        //const fn3 = (this.props.logout);
        //const fn1 = (fn3, this.fn2());
        // const fn2 = (window.location.reload(false));

        const authLinks = (
            <ul className="navbar-nav ml-auto mt-2 mt-lg-0">
                <li className="nav-item"><button className="nav-link btn btn-info btn-sm text-light" onClick={() => {this.fn1();}}>Logout</button></li>
            </ul>
        );
